...
class Node:
    ...
    def select_child(self, c_puct=1.5):
        ...
        for action, child in self.children.items():
            u = child.value() + c_puct * child.prior * math.sqrt(self.visit_count) / (1 + child.visit_count)
            if u > best_score:
                best_score = u
                best_action = action
                best_child = child
...
def run_mcts(env, net, simulations=100, c_puct=1.5):
    ...
    for i, a in enumerate(valid_moves):
        noisy_prior = 0.75 * policy[a] + 0.25 * noise[i]
        root.children[a] = Node(parent=root, prior=noisy_prior)
    ...
    for _ in range(simulations):
        ...
        if not done:
            ...
            logits, value = net(torch.tensor(state).unsqueeze(0))
            policy = logits.exp().squeeze(0).detach().numpy()
            ...
            # Assign value based on the terminal state
            value = 1 if sim_env.check_winner(-sim_env.current_player) else 0
